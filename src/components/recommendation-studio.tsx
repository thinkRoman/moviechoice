...
export default function RecommendationStudio() {
  ...
  const [aiPicks, setAiPicks] = useState<RecommendedTitle[]>([]);
  ...
  const handleAiResults = useCallback((items: Array<{ id: number; mediaType: 'movie' | 'tv'; title: string; year: string | null; rating: number; overview: string; posterUrl: string | null; primaryProvider: string | null; reason: string; kind: 'movie' | 'show' }>, interpretation: string) => {
    const enriched: RecommendedTitle[] = items.map((item) => ({
      id: item.id,
      mediaType: item.mediaType,
      title: item.title,
      ...
      kind: item.kind,
      reason: item.reason,
      providerNames: item.primaryProvider ? [item.primaryProvider] : [],
      primaryProvider: item.primaryProvider,
      score: 0,
      tmdbUrl: item.mediaType === 'movie'
    ...
    <div className="relative min-h-screen bg-[linear-gradient(180deg,#f3eefc_0%,#f7f4ff_38%,#fbfaff_100%)] text-zinc-900">
      ...
      <header className="sticky top-0 z-40 border-b border-violet-100/80 bg-[#f3eefc]/85 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-3xl items-center gap-3 px-4 sm:px-6">
          <Link href="/" className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-violet-600 text-white shadow-md shadow-violet-300/50 transition hover:bg-violet-500">
      ...
      <div className="relative mx-auto max-w-3xl px-4 pb-36 pt-8 sm:px-6">
        ...
        <section className="rounded-[1.75rem] border border-violet-100 bg-white/80 p-5 shadow-[0_18px_50px_-30px_rgba(91,33,182,0.45)] backdrop-blur sm:p-6">
          ...
          <div className="mt-4">
            <NaturalLanguageInput onResults={handleAiResults} disabled={creating || loading} />
          </div>
        ...
        <div id="your-picks" className="scroll-mt-24">
          {showAi && aiPicks.length > 0 && (
            <section className="mb-10 rounded-[1.75rem] border border-violet-200 bg-violet-50/60 p-5 sm:p-6">
              <div className="flex items-center gap-2 text-sm font-black uppercase tracking-[0.22em] text-violet-600">
                <WandSparkles className="h-4 w-4" />
                AI Picks
              </div>
              {aiInterpretation && (
                <p className="mt-2 text-sm text-violet-700">{aiInterpretation}</p>
              )}
              <div className="mt-4 space-y-4">
                {aiPicks.map((item) => (
                  <PickCard key={`ai-${item.mediaType}-${item.id}`} item={item} onHidden={hideTitle} />
                ))}
              </div>
              <button
                type="button"
                onClick={() => { setShowAi(false); setAiPicks([]); setAiInterpretation(''); }}
                className="mt-4 text-xs font-semibold text-violet-500 hover:text-violet-700"
              >
                Clear AI picks
              </button>
            </section>
