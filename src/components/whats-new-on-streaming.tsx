...
export default function WhatsNewOnStreaming() {
  ...
  useEffect(() => {
    setProviderId(readStoredProvider());
    let cancelled = false;
    fetch('/api/recommendations')
      ...
      .then((data) => {
        ...
        setPreferredIds(data.settings.providerIds);
        const stored = readStoredProvider();
        const alreadyChosen = data.settings.providerIds.includes(stored)
          || STREAMING_SERVICES.some((s) => s.id === stored);
        if (!alreadyChosen || !window.localStorage.getItem(STORAGE_KEY)) {
          const firstPreferred = data.settings.providerIds[0];
